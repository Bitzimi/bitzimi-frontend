...
const between=(source,start,end,replacement)=>{const a=source.indexOf(start);const b=source.indexOf(end,a+start.length);if(a<0||b<0)throw new Error(`Coin Flip live-sync marker not found: ${start}`);return source.slice(0,a)+replacement+source.slice(b);};
...
const searchStart='  const startSearch = async () => {';const searchEnd='  useEffect(() => {';
...
const assignStart='  const assignSides = (md?:any) => {';const assignEnd='  useEffect(() => {';
...
const gameStart='  const startGame = (md:any,_assignedPlayerSide:CoinSide) => {';const gameEnd='  const addToSessionHistory =';
...
const uiStart=s.indexOf('              <div className="text-lg text-green-400 font-semibold">Opponent Found!</div>');const uiEndMarker='                <div className="text-sm text-gray-400 mt-4 animate-pulse">Preparing to flip...</div>';const uiEnd=s.indexOf(uiEndMarker,uiStart);
if(uiStart>=0&&uiEnd>=0){let block=s.slice(uiStart,uiEnd+uiEndMarker.length);block=block.replaceAll('myUsername','homeName').replaceAll('identity.avatar','homeAvatar').replaceAll('playerSide','homeSide').replaceAll('opponentName','awayName').replaceAll('opponentAvatar','awayAvatar').replaceAll('opponentSide','awaySide');s=s.slice(0,uiStart)+block+s.slice(uiEnd+uiEndMarker.length);}
...
const modalMarker='          <DialogHeader>\n            <DialogTitle className="text-center">\n              <div className="text-5xl mb-3">';const modalInsert='          <DialogHeader>\n            <DialogTitle className="text-center">\n              <div className="mb-3 text-sm font-semibold text-gray-500 dark:text-gray-400">{myUsername} — {playerSide?.toUpperCase() ?? ""} &nbsp; vs &nbsp; {opponentName} — {opponentSide?.toUpperCase() ?? ""}</div>\n              <div className="text-5xl mb-3">';
if(!s.includes('myUsername} — {playerSide?.toUpperCase()')){if(!s.includes(modalMarker))throw new Error('Coin Flip result modal marker not found');s=s.replace(modalMarker,modalInsert);}
fs.writeFileSync(path,s);console.log('Coin Flip live synchronization and 12s/5s phase repair applied');
