...

{
  ...
  s = s.replace('const hasActiveStake = gameState !== "ready" && gameState !== "idle";\n  const totalPot = matchId ? Number(matchData?.totalPool ?? 0) : (hasActiveStake ? stakeAmount : 0);\n  const winnerGets = matchId ? Number(matchData?.payout ?? 0) : (hasActiveStake && feeRate > 0 ? stakeAmount * (1 - feeRate) : 0);', 'const hasActiveStake = gameState !== "ready" && gameState !== "idle";\n  const totalPot = matchId ? Number(matchData?.totalPool ?? 0) : (hasActiveStake ? stakeAmount : 0);\n  const winnerGets = matchId ? Number(matchData?.payout ?? 0) : (hasActiveStake && feeRate > 0 ? stakeAmount * (1 - feeRate) : 0);');
  s = s.replace('{gameState === "idle" && (\n              <div className="text-center">\n                <Button onClick={() => setGameState("searching")} className="px-10 py-5 text-lg">Search</Button>\n              </div>\n            )}', '{gameState === "ready" && (\n              <div className="text-center">\n                <Button onClick={startSearch} className="h-12 min-w-[190px] px-7 rounded-lg !bg-transparent !text-white font-semibold border border-white/35 hover:!bg-white/[0.06] hover:border-white/55 transition-all duration-200 shadow-none">\n                  <Search className="h-4 w-4 mr-2" />Search\n                </Button>\n                <div className="text-xs text-gray-500 mt-3">Your stake is deducted when you start searching.</div>\n              </div>\n            )}');
