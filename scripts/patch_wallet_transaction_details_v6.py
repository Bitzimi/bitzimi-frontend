...
if 'const getTransactionDetailRows' not in s:
    ...
    s=s[:idx]+helper+'\n'+s[idx:]
block=re.compile(r'\{isExpanded && \(\s*<div className="px-4 pb-4 pt-3 bg-muted/30 dark:bg-white/\[0.02\] border-t border-border">.*?\n\s*\)\}',re.S)
...
print('expanded detail blocks replaced:',n)
if n != 2: raise SystemExit(f'Expected 2 expanded detail blocks, found {n}')
p.write_text(s)
