...
// Backend-derived presentation clock; it never becomes game truth.
s = s.replace(
...
// Remove Coin Flip-specific localStorage truth. Authentication token storage in
// gameMatchmakingService is intentionally untouched.
...
const helpers = `  const timersRef = useRef<ReturnType<typeof setTimeout>[]>([]);
  ...
  const loadHistory = async () => {
    try {
      ...
      setSessionHistory(rows.map((r) => ({ id: r.matchId, opponent: r.opponent.username, opponentAvatar: r.opponent.avatar ?? null, result: r.youWon ? "win" : "loss", outcome: r.result ?? "heads", amount: r.youWon ? r.payout - r.stake : r.stake, stake: r.stake, timestamp: r.settledAt ?? r.createdAt })));
    } catch {}
  };
  ...
  const applySettledResult = async (md: any) => {
    ...
    setPlatformFee(Number(md.platformFee ?? 0)); setCoinResult(result); setIsWinner(won); setWinAmount(won ? winnings : 0);
    setWinnerAvatar(won ? playerAvatar : gameOpponentAvatar); setWinnerName(won ? myUsername : gameOpponentName); setShowWinner(false); setGameState("showing_result");
    ...
    setShowWinner(true); setShowResultPopup(true);
  };
  ...
  const waitForSettlement = (id: string, initialMatch: any) => {
    ...
    const tick = async () => {
      try {
        ...
        if (md.status === "settled" && md.result?.coinFlip && elapsed >= duration) { clearPolling(); await applySettledResult(md); }
      } catch {}
    };
    tick(); pollRef.current = setInterval(tick, 500);
  };
...
const startSearch = `  const startSearch = async () => {
    ...
    searchInFlight.current = true; clearPolling(); clearTimers(); transactionRecorded.current = false;
    ...
    try {
      if (privateMatchId) { const match = await gameMatchmakingService.getMatch(privateMatchId); setMatchId(privateMatchId); setMatchData(match); setOpponentName(match.opponent.username); setOpponentAvatar(match.opponent.avatar ?? match.opponent.username.charAt(0).toUpperCase()); assignSides(match); return; }
      const result = await gameMatchmakingService.joinQueue("pvp_coinflip", stakeAmount);
      if (result.status === "matched" && result.matchId) { const match = await gameMatchmakingService.getMatch(result.matchId); setMatchId(result.matchId); setMatchData(match); setOpponentName(match.opponent.username); setOpponentAvatar(match.opponent.avatar ?? match.opponent.username.charAt(0).toUpperCase()); assignSides(match); return; }
      if (result.queueId) {
        setQueueId(result.queueId);
        pollRef.current = setInterval(async () => {
          try {
            const status = await gameMatchmakingService.pollQueue(result.queueId!);
            if (status.status === "matched" && status.matchId) { clearPolling(); const match = await gameMatchmakingService.getMatch(status.matchId); setMatchId(status.matchId); setMatchData(match); setOpponentName(match.opponent.username); setOpponentAvatar(match.opponent.avatar ?? match.opponent.username.charAt(0).toUpperCase()); assignSides(match); }
...
const assignSides = `  const assignSides = (md?: any) => {
    const data = md ?? matchData; if (!data?.result) return;
    const startedAt = Date.parse(data.createdAt ?? ""); const serverNow = Date.parse(data.serverNow ?? ""); const now = Number.isFinite(serverNow) ? serverNow : Date.now(); const elapsed = Number.isFinite(startedAt) ? Math.max(0, now - startedAt) : 0;
    const assignedPlayerSide: CoinSide = data.isPlayer1 ? data.result.p1Side : data.result.p2Side; const assignedOpponentSide: CoinSide = data.isPlayer1 ? data.result.p2Side : data.result.p1Side;
    setPlayerSide(assignedPlayerSide); setOpponentSide(assignedOpponentSide); setAnimationDurationMs(Number(data.animationDurationMs ?? 8000));
    if (data.status === "settled" && elapsed >= Number(data.animationDurationMs ?? 8000)) { applySettledResult(data); return; }
    if (elapsed >= 3000) { startGame(data, assignedPlayerSide); return; }
    setGameState("side_assignment"); const timer = setTimeout(() => startGame(data, assignedPlayerSide), 3000 - elapsed); timersRef.current.push(timer);
  };

`;
s = between(s, '  const assignSides = (md?: any) => {', '  useEffect(() => {\n    if (!matchId) return;', assignSides);

const startGame = `  const startGame = (md: any, _assignedPlayerSide: CoinSide) => {
    if (!md?.result) return; clearTimers();
    const startedAt = Date.parse(md.createdAt ?? ""); const serverNow = Date.parse(md.serverNow ?? ""); const now = Number.isFinite(serverNow) ? serverNow : Date.now(); const duration = Number(md.animationDurationMs ?? 8000); const elapsed = Number.isFinite(startedAt) ? Math.max(0, now - startedAt) : 0;
    setAnimationDurationMs(duration); setAnimationElapsedMs(Math.min(duration, elapsed)); setCoinResult(md.result.coinFlip as CoinSide); setGameState("flipping");
    if (md.status === "settled" && elapsed >= duration) { applySettledResult(md); return; }
...
console.log("Applied Coin Flip backend-authoritative flow without changing JSX markup.");
