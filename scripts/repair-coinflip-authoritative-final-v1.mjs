...
// Always use one backend-timestamp-derived 17s presentation window:
// 0-12s side assignment, 12-17s coin flip, then reveal the backend result.
...
const helpers=`  const getAuthoritativeElapsedMs=(md:any)=>{const startedAt=Date.parse(md?.createdAt??"");const serverNow=Date.parse(md?.serverNow??"");const now=Number.isFinite(serverNow)?serverNow:Date.now();return Number.isFinite(startedAt)?Math.max(0,now-startedAt):0;};
  ...
  const scheduleCoinFlipTimeline=(md:any)=>{const elapsed=getAuthoritativeElapsedMs(md);const SIDE_MS=12000;const FLIP_MS=5000;const TOTAL_MS=SIDE_MS+FLIP_MS;setAnimationDurationMs(TOTAL_MS);setAnimationElapsedMs(Math.min(TOTAL_MS,elapsed));setPlayerSide(md.isPlayer1?md.result?.p1Side:md.result?.p2Side);setOpponentSide(md.isPlayer1?md.result?.p2Side:md.result?.p1Side);clearTimers();if(elapsed<SIDE_MS){setGameState("side_assignment");timersRef.current.push(setTimeout(()=>scheduleCoinFlipTimeline(md),SIDE_MS-elapsed));return;}if(elapsed<TOTAL_MS){setGameState("flipping");setCoinResult(md.result?.coinFlip??null);timersRef.current.push(setTimeout(()=>scheduleCoinFlipTimeline(md),TOTAL_MS-elapsed));return;}applySettledResult(md);};
...
s=s.slice(0,assignStart)+helpers+'const assignSides = (md?:any) => { const data=md??matchData; if(!data?.result)return; scheduleCoinFlipTimeline(data); };\n\n'+s.slice(assignEnd);

const startMatch=s.match(/const startGame = \(md: any, _assignedPlayerSide: CoinSide\)\s*=>/);
if(startMatch){const start=startMatch.index;const end=s.indexOf('const addToSessionHistory =',start);if(end<0)throw new Error("Coin Flip startGame end marker not found");s=s.slice(0,start)+'const startGame=(md:any,_assignedPlayerSide:CoinSide)=>{if(!md?.result)return;scheduleCoinFlipTimeline(md);};\n\n'+s.slice(end);}

// Backend-only history: retain UI state but never persist Coin Flip history locally.
...
// Generic Home/Player1 vs Away/Player2 mapping. Do not bind the left slot to the logged-in user.
...
fs.writeFileSync(path,s);console.log("Applied final Coin Flip authoritative timeline, generic Home/Away avatar mapping, and local-storage cleanup.");
