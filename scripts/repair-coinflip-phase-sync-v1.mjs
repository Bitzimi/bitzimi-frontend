...
' clearTimers();',
' if(phase==="side_assignment"){setCoinResult(null);setGameState("side_assignment");timersRef.current.push(setTimeout(async()=>{try{await finalScheduleCoinFlipTimeline(await gameMatchmakingService.getMatch(md.matchId));}catch{}},delay));return;}',
' if(phase==="flipping"){setCoinResult(null);setGameState("flipping");timersRef.current.push(setTimeout(async()=>{try{await finalScheduleCoinFlipTimeline(await gameMatchmakingService.getMatch(md.matchId));}catch{}},delay));return;}',
