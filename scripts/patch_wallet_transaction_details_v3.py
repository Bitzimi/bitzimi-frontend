...
helper = r'''  const getTransactionDetailRows = (tx: any, txDate: Date) => {
    ...
    const shortId = typeof tx.id === "string" ? (tx.id.startsWith("tx_") ? tx.id : `tx_${Date.parse(tx.createdAt)}_${tx.id.replace(/-/g, "").slice(0,9)}`) : tx.id;
    const rows: Array<{label:string; value:any}> = [
      {label:"Transaction ID", value:<span className="font-mono text-xs">{shortId}</span>},
...
if 'const getTransactionDetailRows' not in s:
    ...
    s=s[:idx]+helper+'\n'+s[idx:]
pattern=re.compile(r'\[\s*\{\s*label:\s*"Transaction ID".*?\]\s*\.filter\(Boolean\)\s*\.map\(',re.S)
s,n=pattern.subn('[...getTransactionDetailRows(tx, txDate)].filter(Boolean).map(',s)
print('arrays replaced:',n)
if n < 1: raise SystemExit('No transaction detail array found')
p.write_text(s)
