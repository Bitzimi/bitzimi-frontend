...
export function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Registration is handled entirely by the backend.
      //
      // IMPORTANT:
      // - Do not create a local bitzimiUser here.
      // - Do not create a local profile here.
      // - Do not create or store authentication tokens here.
      // - The backend creates the account and sends the verification email.
      //
      // The user must verify the email and then explicitly log in.
      const registered = await registerWithBackend(
        formData.email,
        formData.password,
        formData.username,
      ...
      // Registration succeeded. The backend has created the account and
      // sent the verification email. No authenticated session is created
      // by this page.
          ...
          <CardContent className="pt-10 pb-8 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 mt-2">
                New account registrations are temporarily unavailable. Please
                check back later.
              </p>
            </div>
            <Link
      ...
      <Card className="w-full max-w-md bg-gray-800/50 backdrop-blur-lg border-gray-700/80 shadow-2xl">
        ...
        <CardContent className="pt-0">
            ...
            <Alert className="mb-3 py-2 border-green-500/50 bg-green-950/30">
              ...
              <AlertDescription className="text-green-300 text-xs">
                {formData.affiliateCode ? (
                  <>
                    Joining via affiliate:{" "}
                    <strong>{formData.affiliateCode}</strong>
                  </>
                ) : (
                  <>
                    Joining with referral code:{" "}
                    <strong>{formData.referralCode}</strong>
                  </>
                )}
              </AlertDescription>
            </Alert>
          ...
          {error && (
            <Alert
              variant="destructive"
              className="mb-3 py-2 border-red-500/50 bg-red-950/30"
            >
