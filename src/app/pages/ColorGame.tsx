...
export default function ColorGame() {
  ...
  const { refreshTransactionsFromBackend } = useTransactions();
  const { formatCurrency, convertToUSD, currency } = useSettings();
  const { identity } = useIdentity();
  const { refreshFromBackend: refreshNotifications } = useNotifications();
  ...
  const handleSnapshot = (data: any) => {
    ...
    setGameState(s);
    setBetHistory(data.myBetHistory ?? []);
    ...
    if (prevPhase.current === "spinning" && phase === "result") {
      ...
      refreshTransactionsFromBackend().catch(() => {});
    ...
    if (phase === "result" && processedRound.current !== data.roundNumber) {
      ...
      // A void result is personal: only a user who actually bet receives the
      // void/refund popup. Spectators keep the existing non-participant popup.
      if (data.voided && data.myBet) {
        rd = { won: false, amount: data.myBet.amount, payout: data.myBet.payout ?? data.myBet.amount, team: data.myBet.team, winner: "void" };
      } else if (data.myBet) {
      ...
      setTimeout(() => {
        ...
        if (resultModalTimerRef.current) clearTimeout(resultModalTimerRef.current);
        resultModalTimerRef.current = setTimeout(() => setShowResultModal(false), 5000);
      }, 500);
    }
  };
    ...
    <ResponsiveLayout>
      ...
      <Card className="mb-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white">
        <CardContent className="py-4">
          <div className="flex items-center justify-between">
            <div><p className="text-sm opacity-90">Game Wallet</p><p className="text-2xl font-bold">{formatCurrency(balances.game)}</p></div>
          ...
          return (
            <motion.div key={team} whileHover={{ scale: 1.02 }} transition={{ duration: 0.2 }}>
              <Card className={`${colors.border} bg-gradient-to-br ${colors.bg} overflow-hidden relative`}>
                ...
                <CardHeader className="pb-3"><CardTitle className={`${colors.title} text-lg md:text-xl`}>{team.toUpperCase()}</CardTitle></CardHeader>
                <CardContent className="space-y-2">
                  <div className="flex items-center gap-2 text-sm"><Users className={`h-4 w-4 ${colors.icon} flex-shrink-0`} /><span className="font-medium text-gray-900 dark:text-black">{info.players} players</span></div>
                  <span className={`${amountTextSize} font-bold ${colors.text} break-words block`}>{formatCurrency(info.totalAmount)}</span>
                </CardContent>
              </Card>
            </motion.div>
          );
      ...
      <Card className="mb-4">
        <CardContent className="py-6">
          <div className="text-center space-y-6">
            {gameState.state === "WAITING" && gameState.timeRemaining <= 15 && gameState.timeRemaining > 0 && (
              <motion.div initial={{ opacity: 0, scale: 0.9 }} animate={{ opacity: [1, 0.5, 1], scale: 1 }} transition={{ opacity: { repeat: Infinity, duration: 0.8 } }} className="text-xl font-bold text-amber-600">⚠️ Bet Closing Soon!</motion.div>
            )}
            {gameState.state === "SPINNING" && <div className="text-xl font-bold text-purple-600">🔒 Bet Closed</div>}
            ...
            <AnimatePresence>
              {gameState.state === "RESULT" && gameState.winner && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`text-3xl md:text-4xl font-bold ${gameState.winner === "red" ? "text-red-600" : "text-blue-600"}`}>
      ...
      <Card className="mb-4 bg-gradient-to-r from-yellow-50 to-yellow-100 border-yellow-200">
        <CardContent className="py-4"><div className="text-center"><p className="text-sm text-gray-600 mb-1">Total Pool</p><p className="text-3xl font-bold text-yellow-700 mb-2">{formatCurrency(totalPool)}</p><p className="text-xs text-gray-500">10% platform fee: {formatCurrency(platformFee)}</p></div></CardContent>
      </Card>
      ...
      <Card className="mb-4">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Input type="number" placeholder={`Enter amount (${formatCurrency(minBet)} - ${formatCurrency(maxBet)})`} value={betAmount} onChange={e => { setBetAmount(e.target.value); validateBet(e.target.value); }} disabled={isBettingClosed || placingBet} className={`h-12 text-lg ${betError ? "border-red-500 focus-visible:ring-red-500" : ""}`} />
            {betError && <p className="text-sm text-red-600 font-medium">{betError}</p>}
          </div>

          {!useSlider ? (
            <div className="grid grid-cols-4 gap-2">
              {quickAmounts.map(a => <Button key={a} variant="outline" size="sm" disabled={isBettingClosed || placingBet} onClick={() => { const v = (a * currency.rate).toString(); setBetAmount(v); validateBet(v); }} className="h-10">{formatCurrency(a)}</Button>)}
              <Button variant="outline" size="sm" disabled={isBettingClosed || placingBet} onClick={() => { const v = (maxBet * currency.rate).toString(); setBetAmount(v); validateBet(v); }} className="h-10 font-semibold">Max</Button>
            </div>
          ) : (
            <div className="space-y-2">
              <input type="range" min={Math.floor(minBetLocal)} max={Math.floor(maxBetLocal)} step={Math.max(1, Math.floor((maxBetLocal - minBetLocal) / 100))} value={betAmount || Math.floor(minBetLocal)} onChange={e => { const v = e.target.value; setBetAmount(v); validateBet(v); }} disabled={isBettingClosed || placingBet} className="w-full h-3 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600 disabled:opacity-50" />
              <div className="flex justify-between text-xs text-gray-500"><span>Min: {currency.symbol}{Math.floor(minBetLocal).toLocaleString()}</span><span>Max: {currency.symbol}{Math.floor(maxBetLocal).toLocaleString()}</span></div>
            </div>
          )}

          <div className="grid grid-cols-2 gap-3">
            <Button onClick={() => handleBet("red")} disabled={isBettingClosed || !betAmount || !isBetValid() || placingBet} className="h-12 bg-red-600 hover:bg-red-700 text-white disabled:bg-gray-300 disabled:cursor-not-allowed">{currentRoundBet ? "Bet Placed" : placingBet ? "Placing..." : "Bet on Red"}</Button>
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex border-b">
            {(["your","recent"] as const).map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 pb-3 text-sm font-medium transition-colors ${activeTab === tab ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-500 hover:text-gray-700"}`}>{tab === "your" ? "Your Bet History" : "Recent Bets"}</button>)}
          </div>
        </CardHeader>
        <CardContent>
          {activeTab === "your" ? (
                ...
                {betHistory.map((bet: any) => (
                  <div key={bet.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="font-medium">Round #{bet.roundNumber}</p><p className="text-sm text-gray-600">{formatCurrency(bet.amount)} on {bet.team?.toUpperCase()}</p><p className="text-xs text-gray-500">{new Date(bet.timestamp).toLocaleString()}</p></div>
                    </div>
                    <div className="text-right">
                      {bet.result === "win" ? <div className="flex items-center gap-2"><div><p className="text-sm font-medium text-green-600">WON</p><p className="text-xs text-green-600">+{formatCurrency(bet.payout || 0)}</p></div><CheckCircle className="h-5 w-5 text-green-600" /></div>
                      : bet.result === "loss" ? <div className="flex items-center gap-2"><div><p className="text-sm font-medium text-red-600">LOST</p><p className="text-xs text-red-600">-{formatCurrency(bet.amount)}</p></div><XCircle className="h-5 w-5 text-red-600" /></div>
                      : bet.result === "draw" ? <div className="flex items-center gap-2"><div><p className="text-sm font-medium text-amber-600">REFUNDED</p><p className="text-xs text-amber-600">+{formatCurrency(bet.payout || bet.amount)}</p></div><CheckCircle className="h-5 w-5 text-amber-600" /></div>
          ...
          <DialogHeader>
            <DialogTitle className="text-center text-2xl">
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", duration: 0.5 }}>
                {resultDataRef.current?.winner === "void" ? <span className="text-amber-500">⚠️ ROUND VOID</span>
                : gameState.winner === "red" ? <span className="text-red-600">🔴 RED WINS! 🔴</span>
                : gameState.winner === "blue" ? <span className="text-blue-600">🔵 BLUE WINS! 🔵</span> : null}
              </motion.div>
            </DialogTitle>
