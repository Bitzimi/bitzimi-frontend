...
/**
 * Coin Flip is intentionally a rendering client only.
 * Matchmaking, money movement, result, winner and history all come from the backend.
 * There is no browser persistence for Coin Flip game state.
 */
export default function PvPCoinFlipGame() {
  ...
  const loadMatch = useCallback(async (id: string, animate: boolean) => {
    ...
    const alreadySettled = match.status === "settled" && Boolean(match.result?.coinFlip);
    if (alreadySettled && !animate) {
    ...
    const now = Date.now();
    const wait = Number.isFinite(animationStart) ? Math.max(0, animationStart - now) : 0;
  ...
  const startSearch = useCallback(async () => {
    ...
    try {
      ...
      if (result.status === "matched" && result.matchId) {
        await loadMatch(result.matchId, true);
        return;
      }
        ...
        pollRef.current = setInterval(async () => {
          try {
            ...
            if (status.status === "matched" && status.matchId) {
              clearPolling();
              await loadMatch(status.matchId, true);
            } else if (status.status === "cancelled") {
              clearPolling();
              searchInFlight.current = false;
              setQueueId(null);
              setGameState("ready");
              toast.info("Matchmaking search expired and your stake was returned.");
            }
          } catch {
            // The next poll retries; backend remains authoritative.
          }
        }, 750);
        return;
      }
      ...
      searchInFlight.current = false;
      setGameState("ready");
      toast.error(error?.message || "Unable to start matchmaking");
    }
  }, [clearPolling, clearTimers, loadMatch, stakeAmount]);

  // Backend-only recovery. Reloading cannot create or debit a second search.
  useEffect(() => {
    let disposed = false;
    const recover = async () => {
      if (privateMatchId) {
        try {
          ...
          if (!disposed) {
            searchInFlight.current = true;
            await loadMatch(privateMatchId, false);
          }
        } catch {
          if (!disposed) navigate(roomCode ? `/game/pvp-coinflip/private?roomCode=${roomCode}&stake=${stakeAmount}` : "/game/pvp-coinflip");
        }
        return;
      }
      try {
        ...
        if (recovered.status === "matched" && recovered.matchId) {
          await loadMatch(recovered.matchId, false);
          return;
        }
        if (recovered.status === "waiting" && recovered.queueId) {
          setQueueId(recovered.queueId);
          setGameState("searching");
          pollRef.current = setInterval(async () => {
            try {
        ...
        if (!disposed) setGameState("ready");
      }
    };
    recover();
  ...
  useEffect(() => {
    ...
    fairnessService.getMatchFairness(matchData.matchId).then(setFairnessData).catch(() => {});
  }, [matchData?.matchId]);

  useEffect(() => {
    if (gameState === "showing_result" && matchData?.status === "settled") {
      refreshWalletsFromBackend().catch(() => {});
      loadHistory();
      const timer = setTimeout(() => setShowResultPopup(true), 1500);
  ...
  const handleExit = () => {
    ...
    clearTimers();
    searchInFlight.current = false;
    navigate(roomCode ? `/game/pvp-coinflip/private?roomCode=${roomCode}&stake=${stakeAmount}` : "/game/pvp-coinflip");
  };
  ...
  const totalPool = Number(matchData?.totalPool ?? 0);
  ...
  const feePercent = Number(matchData ? (platformFee / Math.max(totalPool, 1)) * 100 : config.feeRate * 100);

  return (
    <ResponsiveLayout>
      <div className="space-y-3 mb-6">
        <div className="flex items-center">
          <Button variant="ghost" size="sm" onClick={handleExit} className="px-3 -ml-3">
            <ArrowLeft className="h-4 w-4 mr-2" />Back
          </Button>
        </div>
        <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 items-center">
          <div className="min-w-0 flex items-center gap-1.5">
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">Coin Flip</h1>
            <span className="text-sm text-gray-500 whitespace-nowrap">- Stake Room {formatCurrencyNoDecimals(stakeAmount)}</span>
          </div>
          <Button variant="outline" size="sm" onClick={() => setShowRules(v => !v)}><Info className="h-4 w-4 mr-2" />Rules</Button>
          <Button variant="outline" size="sm" onClick={() => setShowFairness(true)} className="justify-self-start"><Shield className="h-3.5 w-3.5 mr-1.5" />Verify Fairness</Button>
        </div>
        {showRules && <Card className="border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50"><div className="p-4"><h3 className="font-semibold mb-2">How to Play</h3><ul className="text-sm text-gray-600 dark:text-gray-300 space-y-1.5 list-disc list-inside"><li>The backend assigns HEADS or TAILS.</li><li>The backend determines the coin result and winner.</li><li>The winner receives the backend-calculated payout after the configured platform fee.</li><li>All wallet movement and history are recorded by the backend.</li></ul></div></Card>}
        <Card className="bg-white/80 dark:bg-gray-900/80 border border-gray-200 dark:border-gray-700"><div className="p-3 sm:p-4"><div className="grid grid-cols-3 gap-2 divide-x divide-gray-200 dark:divide-gray-700"><div className="text-center"><div className="text-[10px] text-gray-500 uppercase mb-1">Bet</div><div className="font-bold">{formatCurrencyNoDecimals(stakeAmount)}</div></div><div className="text-center"><div className="text-[10px] text-gray-500 uppercase mb-1">Pool</div><div className="font-bold">{formatCurrencyNoDecimals(totalPool || stakeAmount * 2)}</div></div><div className="text-center"><div className="text-[10px] text-gray-500 uppercase mb-1">Winner</div><div className="font-bold">{formatCurrencyNoDecimals(winnerPayout)}</div></div></div></div></Card>
      </div>

      <Card className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-[#0a0a0f] dark:to-[#1a1a1a] border border-gray-200 dark:border-gray-800 overflow-hidden">
        <div className="p-6 md:p-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden mb-2"><PlayerAvatar avatar={identity.avatar} /></div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{identity.username}</div></div>
            <div className="flex-1 mx-4 text-center"><div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-200 dark:bg-gray-800/50 rounded px-3 py-1 inline-block">Balance: {formatCurrencyNoDecimals(balances.game)}</div></div>
            <div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden mb-2"><PlayerAvatar avatar={opponent?.avatar ?? opponent?.username?.charAt(0).toUpperCase() ?? "P"} /></div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{opponent?.username ?? "Opponent"}</div></div>
          </div>

          <div className="min-h-[280px] flex flex-col items-center justify-center">
            {gameState === "ready" && <div className="text-center"><Button onClick={startSearch} className="h-12 min-w-[190px] px-7 rounded-lg"><Search className="h-4 w-4 mr-2" />Search for Opponent</Button><div className="text-xs text-gray-500 mt-3">Your stake is deducted only when the backend accepts the search.</div></div>}
            {gameState === "searching" && <div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto mb-4" /><div className="text-base text-gray-300">Searching for opponent...</div><div className="text-xs text-gray-500 mt-2">This search is stored only on the backend.</div></div>}
