...
export default function PvPCoinFlipGame() {
  ...
  const loadHistory = useCallback(async () => { setHistoryLoading(true); try { setHistory(await gameMatchmakingService.getCoinFlipHistory(stakeAmount)); } catch { setHistory([]); } finally { setHistoryLoading(false); } }, [stakeAmount]);
  const loadMatch = useCallback(async (id: string, _animate: boolean) => {
    const match = await gameMatchmakingService.getMatch(id);
    setMatchData(match); setCoinResult((match.result?.coinFlip as CoinSide) ?? null);
    const mine = match.isPlayer1 ? match.result?.p1Side : match.result?.p2Side;
    const opponentCoin = match.isPlayer1 ? match.result?.p2Side : match.result?.p1Side;
    setPlayerSide(mine ?? null); setOpponentSide(opponentCoin ?? null);
    const animationStart = Date.parse(match.animationStartAt ?? match.createdAt ?? "");
    ...
    setAnimationDurationMs(duration); setAnimationElapsedMs(Math.max(0, Math.min(duration, elapsed)));
    ...
    timersRef.current.push(setTimeout(() => {
      ...
      setAnimationElapsedMs(liveElapsed); setGameState("flipping");
  ...
  const startSearch = useCallback(async () => {
    ...
    searchInFlight.current = true; clearPolling(); clearTimers();
    ...
    try {
      ...
      if (result.status === "waiting" && result.queueId) {
        ...
        pollRef.current = setInterval(async () => { try { const status = await gameMatchmakingService.pollQueue(result.queueId!); if (status.status === "matched" && status.matchId) { clearPolling(); await loadMatch(status.matchId, true); } else if (status.status === "cancelled") { clearPolling(); searchInFlight.current = false; setQueueId(null); setGameState("ready"); toast.info("Matchmaking search expired."); } } catch {} }, 750);
      }
    } catch (error: any) { searchInFlight.current = false; setGameState("ready"); toast.error(error?.message || "Unable to start matchmaking"); }
  }, [clearPolling, clearTimers, loadMatch, stakeAmount]);
  useEffect(() => {
    let disposed = false;
    const recover = async () => {
      if (privateMatchId) { try { await gameMatchmakingService.getMatch(privateMatchId); if (!disposed) { searchInFlight.current = true; await loadMatch(privateMatchId, false); } } catch { if (!disposed) navigate(roomCode ? `/game/pvp-coinflip/private?roomCode=${roomCode}&stake=${stakeAmount}` : "/game/pvp-coinflip"); } return; }
      try {
        const recovered = await gameMatchmakingService.recoverQueue("pvp_coinflip", stakeAmount);
        if (disposed || !recovered) return; searchInFlight.current = true;
        if (recovered.status === "matched" && recovered.matchId) { await loadMatch(recovered.matchId, false); return; }
        if (recovered.status === "waiting" && recovered.queueId) { setQueueId(recovered.queueId); setGameState("searching"); pollRef.current = setInterval(async () => { try { const status = await gameMatchmakingService.pollQueue(recovered.queueId!); if (status.status === "matched" && status.matchId) { clearPolling(); await loadMatch(status.matchId, true); } else if (status.status === "cancelled") { clearPolling(); searchInFlight.current = false; setQueueId(null); setGameState("ready"); } } catch {} }, 750); }
      } catch { if (!disposed) setGameState("ready"); }
    };
    recover(); return () => { disposed = true; clearPolling(); clearTimers(); };
  }, [privateMatchId, roomCode, stakeAmount, navigate, clearPolling, clearTimers, loadMatch]);
  useEffect(() => { gameMatchmakingService.getGameConfig("pvp_coinflip").then(c => setConfig({ feeRate: Number(c.feeRate) || 0, stakes: c.stakes || [] })).catch(() => {}); loadHistory(); return () => { clearPolling(); clearTimers(); }; }, [loadHistory, clearPolling, clearTimers]);
  useEffect(() => { if (matchData?.matchId) fairnessService.getMatchFairness(matchData.matchId).then(setFairnessData).catch(() => {}); }, [matchData?.matchId]);
  useEffect(() => { if (gameState !== "showing_result" || matchData?.status !== "settled") return; refreshWalletsFromBackend().catch(() => {}); loadHistory(); const timer = setTimeout(() => setShowResultPopup(true), 1500); timersRef.current.push(timer); return () => clearTimeout(timer); }, [gameState, matchData?.status, refreshWalletsFromBackend, loadHistory]);
  ...
  const hasAuthoritativePositions = Boolean(matchData?.result?.homePlayerId && matchData.result.awayPlayerId);
  const playerIsHome = hasAuthoritativePositions && matchData!.result.homePlayerId === identity.userId;
  const homePlayer = hasAuthoritativePositions ? (playerIsHome ? { avatar: identity.avatar, username: identity.username, coinSide: playerSide } : { avatar: opponent?.avatar ?? null, username: opponent?.username ?? "Opponent", coinSide: opponentSide }) : { avatar: identity.avatar, username: identity.username, coinSide: null };
  const awayPlayer = hasAuthoritativePositions ? (playerIsHome ? { avatar: opponent?.avatar ?? null, username: opponent?.username ?? "Opponent", coinSide: opponentSide } : { avatar: identity.avatar, username: identity.username, coinSide: playerSide }) : { avatar: null, username: "", coinSide: null };
  ...
  return <ResponsiveLayout>
    ...
    <Card className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-[#0a0a0f] dark:to-[#1a1a1a] border border-gray-200 dark:border-gray-800 overflow-hidden"><div className="p-6 md:p-8">
      <div className="flex items-center justify-between mb-6"><div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden mb-2"><PlayerAvatar avatar={homePlayer.avatar ?? "?"} /></div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{homePlayer.username}</div>{hasAuthoritativePositions && <div className="text-[10px] font-semibold uppercase tracking-wider text-blue-500 mt-1">Home</div>}</div><div className="flex-1 mx-4 text-center"><div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-200 dark:bg-gray-800/50 rounded px-3 py-1 inline-block">Balance: {formatCurrencyNoDecimals(balances.game)}</div></div><div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden mb-2">{awayPlayer.username ? <PlayerAvatar avatar={awayPlayer.avatar ?? "?"} /> : <div className="w-full h-full" />}</div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{awayPlayer.username}</div>{hasAuthoritativePositions && <div className="text-[10px] font-semibold uppercase tracking-wider text-red-500 mt-1">Away</div>}</div></div>
      <div className="min-h-[280px] flex flex-col items-center justify-center">
        {gameState === "ready" && <div className="flex items-center justify-center"><Button variant="ghost" onClick={startSearch} className="h-12 min-w-[160px] px-5 rounded-xl border border-gray-400/70 dark:border-gray-500/70 bg-transparent hover:bg-white/10 dark:hover:bg-white/5 hover:border-gray-600 dark:hover:border-gray-300 shadow-sm transition-all"><Search className="h-4 w-4 mr-2" />Search</Button></div>}
        ...
        {gameState === "side_assignment" && <div className="text-center"><div className="text-xl font-bold text-amber-400 mb-6">Sides Assigned!</div><div className="flex justify-center items-center gap-6 md:gap-10 mb-6"><div className="flex flex-col items-center"><div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden mb-3"><PlayerAvatar avatar={homePlayer.avatar ?? "?"} /></div><div className="text-sm font-semibold text-blue-400 mb-2">{homePlayer.username}</div><div className="text-xs font-semibold uppercase tracking-wider text-blue-300 mb-2">Home • {homePlayer.coinSide?.toUpperCase() ?? "—"}</div><div className="text-2xl font-bold bg-blue-700 px-5 py-2.5 rounded-xl border-2 border-blue-400">{homePlayer.coinSide?.toUpperCase() ?? "—"}</div></div><div className="text-3xl font-bold text-gray-400">VS</div><div className="flex flex-col items-center"><div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden mb-3"><PlayerAvatar avatar={awayPlayer.avatar ?? "?"} /></div><div className="text-sm font-semibold text-red-400 mb-2">{awayPlayer.username}</div><div className="text-xs font-semibold uppercase tracking-wider text-red-300 mb-2">Away • {awayPlayer.coinSide?.toUpperCase() ?? "—"}</div><div className="text-2xl font-bold bg-red-700 px-5 py-2.5 rounded-xl border-2 border-red-400">{awayPlayer.coinSide?.toUpperCase() ?? "—"}</div></div></div><div className="text-sm text-gray-400 animate-pulse">Preparing to flip...</div></div>}
        ...
        {gameState === "showing_result" && <div className="text-center"><ProfessionalGoldCoin side={coinResult || "heads"} isAnimating={false} animationElapsedMs={animationDurationMs} animationDurationMs={animationDurationMs} /><div className="text-2xl font-bold text-amber-400 mt-4 mb-6">{coinResult?.toUpperCase()}</div>{showWinner && <div><div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden mx-auto mb-3"><PlayerAvatar avatar={matchData?.youWon ? identity.avatar : opponent?.avatar ?? "?"} /></div><div className={`text-xl md:text-2xl font-bold ${matchData?.youWon ? "text-blue-400" : "text-red-400"}`}>{matchData?.youWon ? identity.username : opponent?.username}</div><div className="text-sm text-gray-400 mt-1">WINS</div></div>}</div>}
      </div>
    </div></Card>
