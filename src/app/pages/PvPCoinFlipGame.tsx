...
type CoinFlipMatchData = MatchResult & { isPlayer1?: boolean };
...
export default function PvPCoinFlipGame() {
  ...
  useEffect(() => {
    if (sessionHistory.length > 0) {
      ...
      } catch (e) { console.error("Error saving session history:", e); }
    }
  ...
  const assignSides = (md?: CoinFlipMatchData) => {
    ...
    setPlayerIsHome(Boolean(data.playerIsHome));
        ...
        try {
          ...
          if (!transactionRecorded.current) {
            ...
            addNotification(
              ...
              won ? `Won ${formatCurrencyNoDecimals(authoritativePayout - stake)} vs ${gameOpponentName} (Result • ${result.toUpperCase()})` : `Lost ${formatCurrencyNoDecimals(stake)} vs ${gameOpponentName} (Result • ${result.toUpperCase()})`,
    ...
    <ResponsiveLayout>
      <div className="space-y-3 mb-6">
        <div className="flex items-center">
          <Button variant="ghost" size="sm" onClick={handleExit} className="hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors px-3 -ml-3"><ArrowLeft className="h-4 w-4 mr-2" /><span className="text-sm font-medium">Back</span></Button>
        </div>
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setShowRules(!showRules)} className="border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white px-4 rounded-lg transition-all shrink-0"><Info className="h-4 w-4 mr-2" />Rules</Button>
          </div>
        </div>
        {showRules && (
          <Card className="border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50"><div className="p-4"><h3 className="font-semibold text-gray-900 dark:text-white mb-2">How to Play</h3><ul className="text-sm text-gray-700 dark:text-gray-300 space-y-1.5 list-disc list-inside"><li>Each player is randomly assigned HEADS or TAILS</li><li>The coin is flipped automatically</li><li>Winner takes 90% of the total pot (10% platform fee)</li><li>Fair random outcome for every game</li></ul></div></Card>
        )}
        <Card className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-gray-200 dark:border-gray-700"><div className="p-3 sm:p-4"><div className="grid grid-cols-3 gap-2 divide-x divide-gray-200 dark:divide-gray-700"><div className="text-center px-1"><div className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Bet</div><div className="text-sm md:text-base font-bold text-gray-900 dark:text-white">{formatCurrencyNoDecimals(stakeAmount)}</div></div><div className="text-center px-1"><div className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Pool</div><div className="text-sm md:text-base font-bold text-gray-900 dark:text-white">{formatCurrencyNoDecimals(displayedPool)}</div></div><div className="text-center px-1"><div className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Winner</div><div className="text-sm md:text-base font-bold text-gray-900 dark:text-white">{formatCurrencyNoDecimals(displayedWinner)}</div></div></div></div></Card>
      </div>
      ...
      <Card className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-[#0a0a0f] dark:to-[#1a1a1a] border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
        <div className="p-6 md:p-8">
          <div className="flex items-center justify-between mb-6">
            {playerIsHome ? (
              <>
                <div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-2xl md:text-3xl mb-2 overflow-hidden"><PlayerAvatar avatar={identity.avatar} /></div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{myUsername}</div></div>
                ...
                <div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center text-2xl md:text-3xl mb-2 overflow-hidden"><PlayerAvatar avatar={opponentAvatar} /></div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{opponentName}</div></div>
              </>
            ) : (
              <>
                <div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center text-2xl md:text-3xl mb-2 overflow-hidden"><PlayerAvatar avatar={opponentAvatar} /></div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{opponentName}</div></div>
          ...
          <div className="min-h-[280px] flex flex-col items-center justify-center">
            ...
            {gameState === "side_assignment" && (
              <div className="text-center">
                ...
                <div className="flex justify-center items-center gap-6 md:gap-10 mb-6">
                  {playerIsHome ? (
                    <>
                      <div className="flex flex-col items-center"><div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-3xl md:text-4xl mb-3 shadow-xl shadow-blue-500/50 ring-4 ring-blue-400/30 overflow-hidden"><PlayerAvatar avatar={identity.avatar} /></div><div className="text-sm font-semibold text-blue-400 mb-2">{myUsername}</div><div className="text-2xl md:text-3xl font-bold text-white bg-gradient-to-r from-blue-600 to-blue-700 px-5 py-2.5 rounded-xl shadow-lg border-2 border-blue-400">{playerSide?.toUpperCase()}</div></div>
                      <div className="text-3xl md:text-4xl font-bold text-gray-400">VS</div>
                      <div className="flex flex-col items-center"><div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center text-3xl md:text-4xl mb-3 shadow-xl shadow-red-500/50 ring-4 ring-red-400/30 overflow-hidden"><PlayerAvatar avatar={opponentAvatar} /></div><div className="text-sm font-semibold text-red-400 mb-2">{opponentName}</div><div className="text-2xl md:text-3xl font-bold text-white bg-gradient-to-r from-red-600 to-red-700 px-5 py-2.5 rounded-xl shadow-lg border-2 border-red-400">{opponentSide?.toUpperCase()}</div></div>
                    </>
                  ) : (
                    <>
                      <div className="flex flex-col items-center"><div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center text-3xl md:text-4xl mb-3 shadow-xl shadow-red-500/50 ring-4 ring-red-400/30 overflow-hidden"><PlayerAvatar avatar={opponentAvatar} /></div><div className="text-sm font-semibold text-red-400 mb-2">{opponentName}</div><div className="text-2xl md:text-3xl font-bold text-white bg-gradient-to-r from-red-600 to-red-700 px-5 py-2.5 rounded-xl shadow-lg border-2 border-red-400">{opponentSide?.toUpperCase()}</div></div>
            ...
            {gameState === "showing_result" && (
              <div className="text-center">
                <div className="mx-auto mb-3"><ProfessionalGoldCoin side={coinResult || "heads"} isAnimating={false} /></div>
                <div className="text-2xl font-bold text-amber-400 mb-6">{coinResult?.toUpperCase()}</div>
                {showWinner && (<div className="mt-6"><div className={`w-20 h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center text-4xl md:text-5xl mx-auto mb-3 shadow-lg overflow-hidden ${isWinner ? "bg-gradient-to-br from-blue-500 to-blue-600 shadow-blue-500/50" : "bg-gradient-to-br from-red-500 to-red-600 shadow-red-500/50"}`}><PlayerAvatar avatar={isWinner ? identity.avatar : winnerAvatar} /></div><div className={`text-xl md:text-2xl font-bold ${isWinner ? "text-blue-400" : "text-red-400"}`}>{winnerName}</div><div className="text-sm text-gray-400 mt-1">WINS</div></div>)}
              </div>
            )}
          </div>
        </div>
      ...
      {sessionHistory.length > 0 && (<Card className="mt-4 bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 shadow-sm"><div className="p-4"><h3 className="text-sm font-semibold text-gray-900 dark:text-gray-300 mb-3">Your History (${stakeAmount} Stake)</h3><div className="overflow-x-auto"><div className="space-y-2 min-w-max">{sessionHistory.map((record) => (<div key={record.id} className="flex items-center gap-2 whitespace-nowrap p-2 rounded bg-gray-100 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50"><div className="w-8 h-8 shrink-0 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 dark:from-gray-600 dark:to-gray-700 flex items-center justify-center text-sm overflow-hidden"><PlayerAvatar avatar={record.playerAvatar || identity.avatar} /></div><span className="text-xs font-medium text-gray-900 dark:text-gray-200">{record.playerName || myUsername}</span><span className="text-xs text-gray-500">—</span><span className={`text-xs font-semibold px-2 py-0.5 rounded ${record.result === "win" ? "bg-green-500/20 text-green-600 dark:text-green-400" : "bg-red-500/20 text-red-600 dark:text-red-400"}`}>{record.result === "win" ? "WON" : "LOST"}</span><span className={`text-sm font-bold ${record.result === "win" ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>{record.result === "win" ? "+" : "-"}{formatCurrencyNoDecimals(record.amount)}</span><span className="text-xs text-gray-500">vs</span><div className="w-8 h-8 shrink-0 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 dark:from-gray-600 dark:to-gray-700 flex items-center justify-center text-sm overflow-hidden"><PlayerAvatar avatar={record.opponentAvatar || record.opponent.charAt(0).toUpperCase()} /></div><span className="text-xs text-gray-600 dark:text-gray-300">{record.opponent}</span><span className="text-xs text-gray-500">Result: {record.outcome.toUpperCase()}</span></div>))}</div></div></div></Card>)}
      ...
      <style>{`@keyframes flipCoin { 0% { transform: rotateY(0deg); } 100% { transform: rotateY(1800deg); } }`}</style>
    </ResponsiveLayout>
  );
}
