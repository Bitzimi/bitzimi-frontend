    ...
    <ResponsiveLayout>
      <div className="space-y-3 mb-6">
        <div className="flex items-center">
          <Button variant="ghost" size="sm" onClick={() => navigate("/game/pvp-coinflip")} className="hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors px-3 -ml-3"><ArrowLeft className="h-4 w-4 mr-2" /><span className="text-sm font-medium">Back</span></Button>
        </div>
      ...
      {sessionHistory.length > 0 && (<Card className="mt-4 bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 shadow-sm"><div className="p-4"><h3 className="text-sm font-semibold text-gray-900 dark:text-gray-300 mb-3">Session History (${stakeAmount} Stake)</h3><div className="space-y-2">{sessionHistory.map((record) => (<div key={record.id} className="flex items-center justify-between p-2 rounded bg-gray-100 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50"><div className="flex items-center gap-2 flex-1"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 dark:from-gray-600 dark:to-gray-700 flex items-center justify-center text-sm overflow-hidden"><PlayerAvatar avatar={record.result === "win" ? identity.avatar : record.opponent.charAt(0).toUpperCase()} /></div><span className={`text-xs font-semibold px-2 py-0.5 rounded ${record.result === "win" ? "bg-green-500/20 text-green-600 dark:text-green-400" : "bg-red-500/20 text-red-600 dark:text-red-400"}`}>{record.result === "win" ? "WIN" : "LOSS"}</span><span className="text-xs text-gray-600 dark:text-gray-400">{record.result === "win" ? myUsername : record.opponent}</span><span className="text-xs text-gray-500">• {record.outcome.toUpperCase()}</span></div><div className={`text-sm font-bold ${record.result === "win" ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>{record.result === "win" ? "+" : "-"}{formatCurrencyNoDecimals(record.amount)}</div></div>))}</div></div></Card>)}
