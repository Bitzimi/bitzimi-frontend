...
export default function PvPCoinFlipGame() {
  ...
  const playerIsHome = Boolean(matchData?.result?.homePlayerId && matchData.result.homePlayerId === identity.userId);
  const homePlayer = playerIsHome ? { avatar: identity.avatar, username: identity.username, coinSide: playerSide } : { avatar: opponent?.avatar ?? null, username: opponent?.username ?? "Opponent", coinSide: opponentSide };
  const awayPlayer = playerIsHome ? { avatar: opponent?.avatar ?? null, username: opponent?.username ?? "Opponent", coinSide: opponentSide } : { avatar: identity.avatar, username: identity.username, coinSide: playerSide };
  ...
  return <ResponsiveLayout>
    ...
    <Card className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-[#0a0a0f] dark:to-[#1a1a1a] border border-gray-200 dark:border-gray-800 overflow-hidden"><div className="p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden mb-2"><PlayerAvatar avatar={homePlayer.avatar ?? undefined} /></div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{homePlayer.username}</div>{hasAuthoritativePositions && <div className="text-[10px] font-semibold uppercase tracking-wider text-blue-500 mt-1">Home</div>}</div>
        ...
        <div className="flex flex-col items-center"><div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden mb-2"><PlayerAvatar avatar={awayPlayer.avatar ?? undefined} /></div><div className="text-xs text-gray-600 dark:text-gray-400 font-medium">{awayPlayer.username}</div>{hasAuthoritativePositions && <div className="text-[10px] font-semibold uppercase tracking-wider text-red-500 mt-1">Away</div>}</div>
      </div>
      <div className="min-h-[280px] flex flex-col items-center justify-center">
        ...
        {gameState === "side_assignment" && <div className="text-center"><div className="text-xl font-bold text-amber-400 mb-6">Sides Assigned!</div><div className="flex justify-center items-center gap-6 md:gap-10 mb-6"><div className="flex flex-col items-center"><div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden mb-3"><PlayerAvatar avatar={homePlayer.avatar ?? undefined} /></div><div className="text-sm font-semibold text-blue-400 mb-2">{homePlayer.username}</div><div className="text-xs font-semibold uppercase tracking-wider text-blue-300 mb-2">Home • {homePlayer.coinSide?.toUpperCase() ?? "—"}</div><div className="text-2xl font-bold bg-blue-700 px-5 py-2.5 rounded-xl border-2 border-blue-400">{homePlayer.coinSide?.toUpperCase() ?? "—"}</div></div><div className="text-3xl font-bold text-gray-400">VS</div><div className="flex flex-col items-center"><div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden mb-3"><PlayerAvatar avatar={awayPlayer.avatar ?? undefined} /></div><div className="text-sm font-semibold text-red-400 mb-2">{awayPlayer.username}</div><div className="text-xs font-semibold uppercase tracking-wider text-red-300 mb-2">Away • {awayPlayer.coinSide?.toUpperCase() ?? "—"}</div><div className="text-2xl font-bold bg-red-700 px-5 py-2.5 rounded-xl border-2 border-red-400">{awayPlayer.coinSide?.toUpperCase() ?? "—"}</div></div></div><div className="text-sm text-gray-400 animate-pulse">Preparing to flip...</div></div>}
        ...
        {gameState === "showing_result" && <div className="text-center"><ProfessionalGoldCoin side={coinResult || "heads"} isAnimating={false} animationElapsedMs={animationDurationMs} animationDurationMs={animationDurationMs} /><div className="text-2xl font-bold text-amber-400 mt-4 mb-6">{coinResult?.toUpperCase()}</div>{showWinner && <div><div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden mx-auto mb-3"><PlayerAvatar avatar={matchData?.youWon ? identity.avatar : opponent?.avatar ?? undefined} /></div><div className={`text-xl md:text-2xl font-bold ${matchData?.youWon ? "text-blue-400" : "text-red-400"}`}>{matchData?.youWon ? identity.username : opponent?.username}</div><div className="text-sm text-gray-400 mt-1">WINS</div></div>}</div>}
      </div>
    </div></Card>
