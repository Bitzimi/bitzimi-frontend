...
export function TaskModal({ task, isVIP, open, onClose, onComplete }: TaskModalProps) {
  ...
  const { addTransaction, refreshWalletsFromBackend } = useWallet();
      ...
      const poll = setInterval(async () => {
        ...
        try {
          ...
          if (upd.status === "approved" || upd.status === "admin_approved") {
            ...
            addTransaction("task_completed", rew, "task", `Task completed: ${task.title}`, {taskId:task.id,categoryId:task.type});
